/**
 * get_bahraini_implementations — Find Bahraini statutes implementing a specific EU directive/regulation
...
export interface GetBahrainiImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface BahrainiImplementationResult {
  document_id: string;
...
export async function getBahrainiImplementations(
  db: InstanceType<typeof Database>,
  input: GetBahrainiImplementationsInput,
): Promise<ToolResponse<BahrainiImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as BahrainiImplementationResult[];
