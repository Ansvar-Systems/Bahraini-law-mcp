/**
 * search_eu_implementations — Search EU directives/regulations that have Bahraini implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_bahraini_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as bahraini_statute_count
  ...
  if (input.has_bahraini_implementation) {
    sql += ' HAVING bahraini_statute_count > 0';
  }
