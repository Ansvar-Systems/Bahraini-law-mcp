#!/usr/bin/env tsx
/**
 * Bahraini Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://legalaffairs.gov.bh';
const PORTAL_NAME = 'Bahrain Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/bahraini-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Bahraini Law MCP — Data Freshness Check');
