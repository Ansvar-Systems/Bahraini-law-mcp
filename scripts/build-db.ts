#!/usr/bin/env tsx
/**
 * Database builder for Bahrain Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Bahrain Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Bahraini statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'BH');
    insertMeta.run('source', 'legalaffairs.gov.bh');
