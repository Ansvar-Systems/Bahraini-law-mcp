#!/usr/bin/env tsx
/**
 * Drift detection for Bahrain Law MCP.
 *
 * Checks if upstream Bahrain Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Bahrain Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'bahraini-law-mcp/1.0 drift-detect' },
      });
